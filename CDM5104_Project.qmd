...
# 2. Datasets and required software

### Datasets

The data files used in this project can be found in the folder "data". Please unzip the files "Plasma_QC_1_POS.mzML.zip" and "Plasma_QC_1_NEG.mzML.zip" before using them in the next section.
...
Start mzmine (see Section 1) and import the raw data file "Plasma_QC-1_POS.mzML" (see previous section 2) by drag and drop to the tab "MS Data Files". Right-click on the file name and select "Show 3D plot", click on "Auto Range", and then "OK"
...
**Note:** The screenshot/view shown is from MZmine running on **macOS (**4.8.30). On other operating systems or MZmine versions, the BPI chromatogram may appear different, and the displayed *m/z* values may vary. Until this page is fully updated, please refer to the project guidance session for additional details.
...
**(c)** Now look at the base peak intensity (BPI) chromatogram. Select the m/z values of the two highest peaks (876.8005 and 758.5688) and search for possible lipid species matching using the LIPID MAPS database (**Important**: Select **"All Positive Ions"**). Describe the results of this search and discuss why multiple matches were returned for one of the m/z values and if an assignment can be made at this stage.
:::
